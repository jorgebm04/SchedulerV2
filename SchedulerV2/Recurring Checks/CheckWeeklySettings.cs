...
{
    public static class CheckWeeklySettings
    {
        public static void CheckRecurrentWeeklySettings(Settings settings)
        {
            if (settings.occursOnceAt)
            {
                settings.needToAddDay = OccursOnceAtChecker.CheckOccursOnceAt(settings.currentDate, settings.occursOnceAtHour);
            }
            else if (settings.occursEvery)
            {
                settings.needToAddDay = OccursOnceAtChecker.CheckOccursOnceAt(settings.currentDate, settings.endingHour);
            ...
            {
                settings.nextExecutionTime = "Select a daily frequency";
                return;
            }
        ...
        public static void CheckDays(Settings settings)
        {
            int lastDay = DaysChecker.CheckDays(settings);
            if (lastDay == -1)
            {
                settings.nextExecutionTime = "Select the day(s) of the week";
                return;
            }
            settings.lastDay = lastDay;
